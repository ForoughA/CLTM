function [nodePot,edgePot,Params,ll] = EMtest_stoch(adjmat,samples,covariates,options)
...
for iter = 1:options.max_ite
    ...
    %E-Step
    for t = 1:nSamples
        weighted_prob(t,:) = samples(:,t)' * covariates(:,:,t);% samples(:,t) .* sum(covariates(:,:,t),2);
        nodePot(1:nObs,2) = exp(covariates(:,:,t) * Params);
...
%         nodePot(indSample2,2) = 1; 
    ...
    while ~done
        ...
        grad = 0;
        stp = zeros(nSamples,1);
        acc = zeros(nSamples,1);
        VPold = VPnew;
        for t=1:nSamples
            tmp = compute_gradient(adjmat,nodePot,edgePot,Params,samples(:,t),covariates(:,:,t),weighted_prob(t,:),tree_msg_order);
            Params = Params - stepi * grad;
           stp(t) = (stepi*norm(grad,2));
           grad = grad + tmp;
           if t~=1 && t~=nSamples
            [VP,CP,CA] = pred(nodePot,edgePot,adjmat,covariates(:,:,t-1:t),samples(:,t:t+1),Params);
            acc(t) = mean(VP);
           end
        end
        VPnew = mean(acc(2:end-1));
        fprintf('EM iteration: %d, gradient iteration: %d, gradient update: %f \n',iter,GDiter,mean(stp));
        GDiter = GDiter + 1;
        
        done = ( abs(VPold-VPnew) < epsilon) || (GDiter >= maxSteps) ;
    end
