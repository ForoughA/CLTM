function [adjmatTree, edge_distance, nodePot, edgePot, ll, bic] = condRegCLRG_discrete2(samples, options,covariates)
...
[cov_prob_Xk,prob_bij,mar_prob] = computeBnCondStats(samples,covariates);
[distance,cond_prob] = computeCondProbs(prob_bij,cov_prob_Xk,mar_prob);
...
[nodePot, edgePot] = marToPotBin(cond_prob,tree_msg_order);
...
while(num_nodes <= m+options.maxHidden)
    ...
    for j=1:length(list_cand_nodes)
        ...
        [adjmatS, edge_distanceS] = RG2(distance(i_family,i_family),1, numSamples);
