function [Params,dParams,llEM,EcllEM] = EM_UGMbothStoch(adjmat,samples,covariates,depCovariates,options)
...
%             Gk = zeros(nCov,1)';%graident update for the node potential
            ...
            for e=1:nEdges
                ...
                for ne = 1:ndepCov
                    if par<=nObs && child<=nObs
                        Gl(ne) = ((samples(par,t)-1)*(samples(child,t)-1)- e_mar(2,2,e))*depCovariates(par,child,ne,t);
                    elseif par>nObs && child<=nObs
                        Gl(ne) = ((samples(child,t)-1)*(node_marginal{t}(par,:)*[0 1]') - e_mar(2,2,e))*depCovariates(par,child,ne,t);
                    elseif par<=nObs && child>nObs
                        Gl(ne) = ((samples(par,t)-1)*(node_marginal{t}(child,:)*[0 1]') - e_mar(2,2,e))*depCovariates(par,child,ne,t);
                    elseif par>nObs && child>nObs
                        Gl(ne) = (edge_marginal{t}(2,2,e) - e_mar(2,2,e))*depCovariates(par,child,ne,t);
                    end
                end
            end
    ...
    t = toc;
    fprintf('time taken during the M-step is: %f gradIter: %d\n,',t,iter);
