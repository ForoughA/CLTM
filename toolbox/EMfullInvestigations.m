function [nodePot,edgePot,Params,ll] = EMfullInvestigations(adjmat,samples,covariates,options)
...
elseif(isfield(options,'edge_distance')) % Initialize parameters with edge distance
    ...
    Params = rand(nCov,1);
%     Params = Params / (sum(Params([1,7]))+max(Params(2:6)));
else
...
for iter = 1:options.max_ite
    ...
    %E-Step
    for t = 1:nSamples
        weighted_prob(t,:) = (samples(:,t))' * (covariates(1:nObs,:,t)+1);% samples(:,t) .* sum(covariates(:,:,t),2);
...
%         nodePot(nObs+1:nTotal,1) = 1 - nodePot(nObs+1:nTotal,2);
    ...
    for t=1:nSamples
        weighted_prob(t,:) = weighted_prob(t,:) + (hiddenNodeStats(nObs+1:nTotal,:)*[1 2]')'*(covariates(nObs+1:nTotal,:,t)+1);
    end
...
%         Phi_i(:,1) = 1 - Phi_i(:,2);
        ...
        for ki = 1:nParam
            G(ki) = first_term(ki) - (cov(:,ki)+1)' * (node_mar*[1 2]');
        end
end
