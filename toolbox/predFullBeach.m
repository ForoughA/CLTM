...
function [VP,CP,CA,EA] = predFullBeach(Ck,Dk,edgePot,adj,cov,eCov,samples,eSamples)
% Ck = abs(Ck);
% Ck = Ck/max(Ck);
...
%     cov(:,end-1,t+1) = Prediction(:,t)-1;
%     edgeCov(:,5,t) = repmat(log(sum(Prediction(:,t)-1)+eps),[nEdges,1]);
%     eCov(:,:,6,t) = repmat(log(sum(Prediction(:,t)-1)+eps),[nObs,nObs]);
end
...
%            eCov(subEdgeInd(i),subEdgeInd(j),7,t+1) = round(logistic(reshape(eCov(subEdgeInd(i),subEdgeInd(j),:,t),[1,nEcov+1]),Dk));
           ctr = ctr + 1;
       end
   end
