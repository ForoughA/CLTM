function [adjmatTree, edge_distance] = regCLRG_Gaussian(distance, numSamples, verbose, maxHidden, thrsh)
...
while(~isempty(internal_nodes) && num_nodes <= m+maxHidden)
    ...
    for j=1:length(internal_nodes)
        ...
        [adjmatS, edge_distanceS] = RG(distance(i_family,i_family),1, numSamples, thrsh);
