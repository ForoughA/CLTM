function [NLL,g] = UGM_HCRF_NLL_Forough(w,Xnode,Xedge,Y,nodeMap,edgeMap,nHidden,edgeStruct,inferFunc,varargin)
...
NLL = 0;
g = zeros(size(w));
nodePot = zeros(nNodes+nHidden,2);
...
for i = 1:nInstances
    ...
    if edgeStruct.useMex
        [nodePot(1:nNodes,:),edgePot] = UGM_CRF_makePotentialsC(w,Xnode,Xedge,nodeMap,edgeMap,nStates,edgeEnds,int32(i));
    else
        [nodePot(1:nNodes,:),edgePot] = UGM_CRF_makePotentials(w,Xnode,Xedge,nodeMap,edgeMap,edgeStruct,i);
    end
%     nodePot(nNodes+1:nNodes+nHidden,:) = 
